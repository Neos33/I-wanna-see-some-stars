...
@export_enum("None:1", "Stop:2", "Bounce:3", "Custom:4") var collision_interaction: int = 4
var start_moving: bool = false
var movement_velocity: int
...
func _ready():
	...
	movement_velocity = abs(move_speed.x + move_speed.y)
...
func _on_area_2d_body_entered(body: Node2D) -> void:
	if collision_interaction == 4:
		var get_direction = body.direction_target
		var apply_speed = Vector2.ZERO
		match get_direction:
			0:
				move_speed = Vector2(movement_velocity, 0)
			90:
				move_speed = Vector2(0, movement_velocity)
			180:
				move_speed = Vector2(-movement_velocity, 0)
			270:
				move_speed = Vector2(0, -movement_velocity)
